...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	/* 比以前低 */
		if (height >= 0) {
			...
			sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, 0); /* 按新图层的信息重新绘制画面 */
		}
		...
		sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, height); /* 按新图层重新绘制画面 */
	}
	return;
}
